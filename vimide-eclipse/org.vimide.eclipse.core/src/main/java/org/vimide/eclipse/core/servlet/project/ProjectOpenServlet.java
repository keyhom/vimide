 ...
 */
@WebServlet(urlPatterns = "/project_open")
public class ProjectOpenServlet extends GenericVimideHttpServlet {

    private static final long serialVersionUID = 1L;
    static final Logger LOGGER = LoggerFactory
            .getLogger(ProjectOpenServlet.class);
        ...
        for (IProject project : projects) {
            ...
            if (project.exists()) {
                try {
                    ...
                    sb.append(NLS.bind(CoreMessages.project_opened,
                            project.getName()));
                } catch (CoreException e) {
                    LOGGER.error("Project '{}' opening exception: {}",
                            project.getName(), e.getMessage(), e);
                    sb.append(NLS.bind(CoreMessages.project_op_failed,
                            "Opening", project.getName()));
                }
            } else {
